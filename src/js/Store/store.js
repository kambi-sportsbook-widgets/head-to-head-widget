...
const parseDataInfo = function(statistics) {
  ...
  statistics.lastEvents.forEach(function(lastEvent) {
    ...
    for (let j = 0; j < lastEvent.scores.length; j++) {
      if (
        lastEvent.scores[j].hasOwnProperty('homeScore') &&
        lastEvent.scores[j].hasOwnProperty('awayScore')
      ) {
        allScores.push(
          lastEvent.scores[j].homeScore,
          lastEvent.scores[j].awayScore
        )
      }
    }

    if (
      lastEvent.scores.length &&
      lastEvent.scores[0].hasOwnProperty('homeScore') &&
      lastEvent.scores[0].hasOwnProperty('awayScore')
    ) {
...
const getEventStatistics = function(eventId) {
  ...
  return statisticsModule.getHeadToHeadStatistics(eventId).then(parseDataInfo)
}
